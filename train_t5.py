...
def eval_epoch(args, model, dev_loader, gt_sql_pth, model_sql_path, gt_record_path, model_record_path):
    ...
    with torch.no_grad():
        for encoder_input, encoder_mask, decoder_input, decoder_targets, _ in tqdm(dev_loader, desc="Computing dev loss"):
            encoder_input = encoder_input.to(DEVICE)
            encoder_mask = encoder_mask.to(DEVICE)
            decoder_input = decoder_input.to(DEVICE)
            decoder_targets = decoder_targets.to(DEVICE)
    ...
    # Generate SQL queries for metrics computation
    tokenizer = T5TokenizerFast.from_pretrained('google-t5/t5-small')
    with torch.no_grad():
        for encoder_input, encoder_mask, _, _, initial_decoder_input in tqdm(dev_loader, desc="Generating SQL queries"):
            encoder_input = encoder_input.to(DEVICE)
            encoder_mask = encoder_mask.to(DEVICE)
    ...
    # Save the model predictions and compute records
    ...
    return total_loss / (total_tokens + 1e-8), record_f1, record_em, sql_em, error_rate
...
def test_inference(args, model, test_loader, model_sql_path, model_record_path):
    ...
    with torch.no_grad():
        for encoder_input, encoder_mask, initial_decoder_input in tqdm(test_loader, desc="Generating test SQL queries"):
            encoder_input = encoder_input.to(DEVICE)
            encoder_mask = encoder_mask.to(DEVICE)
            initial_decoder_input = initial_decoder_input.to(DEVICE)
    ...
    print(f"Generated {len(all_predictions)} SQL queries for test set")
